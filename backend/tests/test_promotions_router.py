...
async def test_promotions_503_when_promotions_off(client: AsyncClient) -> None:
    """Promotions require enable_promotions flag; off → 503."""
    _flag_cache["enable_promotions"] = False
    resp = await client.get("/api/promotions")
    assert resp.status_code == 503


async def test_promotions_ok_when_promotions_on(client: AsyncClient) -> None:
    """When enable_promotions is on, promotions list returns 200/empty."""
    _flag_cache["enable_promotions"] = True
    resp = await client.get("/api/promotions")
    assert resp.status_code == 200
