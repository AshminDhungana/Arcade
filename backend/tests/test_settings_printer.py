"""Tests for printer config keys persisted via PATCH /api/settings (Task 24)."""
...
@pytest.mark.asyncio
async def test_patch_printer_type_persists_and_is_returned(client: AsyncClient):
    res = await client.patch("/api/settings", json={"printer_type": "network"})
    assert res.status_code == 200
    res2 = await client.get("/api/settings")
    assert res2.status_code == 200
    assert res2.json().get("printer_type") == "network"
