...
export class LinuxPlatformService implements IPlatformService {
  ...
  private startHotspotPolling(): void {
    ...
    this.hotspotTimer = setInterval(() => this.pollHotspot(), LinuxPlatformService.HOTSPOT_POLL_MS);
  }
  ...
  private pollHotspot(): void {
    ...
    if (inWindow) {
      if (this.hotspotZone === 'minimal') {
        ...
        active = cx >= bounds.width - LinuxPlatformService.HOT_ZONE_WIDTH;
      } else {
        // Bottom-right corner (matches the 20x20 CSS trigger zone)
        active =
          cx >= bounds.width - LinuxPlatformService.HOT_ZONE_WIDTH &&
          cy >= bounds.height - LinuxPlatformService.HOT_ZONE_WIDTH;
      }
    }
