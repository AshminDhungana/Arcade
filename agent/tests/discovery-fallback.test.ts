...
describe('discoverServer HTTP fallback', () => {
  ...
  it('returns ws URL from HTTP /api/discovery when UDP times out', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('http://192.168.1.1:80/api/agent/discovery', expect.any(Object));
    expect(mockFetch).toHaveBeenCalledWith('http://192.168.0.1:80/api/agent/discovery', expect.any(Object));
    expect(mockFetch).toHaveBeenCalledWith('http://192.168.1.254:80/api/agent/discovery', expect.any(Object));
  });
